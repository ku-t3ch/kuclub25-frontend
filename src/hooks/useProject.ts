...
const normalizeProject = (() => {
  const cache = new WeakMap<Project, Project>();
  ...
  return (project: Project): Project => {
    ...
    Object.defineProperties(normalized, {
      startDateFormatted: {
        get() {
          return formatDateForDisplay(
            this.date_start_the_project || this.date_start
          );
        },
        enumerable: false,
        configurable: true,
      ...
      endDateFormatted: {
        get() {
          return formatDateForDisplay(
            this.date_end_the_project || this.date_end
          );
        },
        enumerable: false,
        configurable: true,
    ...
    cache.set(project, normalized);
    return normalized;
  };
})();

// Optimized generic hook with abort controller
...
export const useProjects = (): ProjectAllReturn => {
  ...
  const {
    ...
    refetch,
    clearError,
  } = useApiRequest(fetchProjects, [], true);
...
export const useProjectDetail = (id: string | null): ProjectDetailReturn => {
  ...
  const {
    ...
    refetch,
    clearError,
  } = useApiRequest(fetchProject, [id], !!id);
...
export const useProjectsByOrganization = (
  ...
  const fetchProjectsByOrganization = useCallback(async (): Promise<
    Project[]
  > => {
    if (!orgId) return [];
    ...
    if (!response.success || !response.data) {
      throw new Error(
        response.message || "Failed to fetch projects by organization"
      );
    }
  ...
  const {
    ...
    refetch,
    clearError,
  } = useApiRequest(fetchProjectsByOrganization, [orgId], !!orgId);
...
// Regular functions (not using hooks) for filtering, sorting, and pagination
const createFilterProjectsFunction = () => {
  const cache = new Map<string, Project[]>();

  return (projects: Project[], filters: ProjectFilters): Project[] => {
    ...
    const result = projects.filter((project) => {
      ...
      if (
        ...
        project.organization_orgid !== filters.organizationId
      ) {
        return false;
      }
      ...
      if (filters.activityFormat) {
        ...
        if (typeof projectFormat === "string") {
          ...
          if (!projectFormat.some((f) => f.toLowerCase().includes(format)))
            return false;
        } else {
          return false;
        }
      ...
      if (filters.location) {
        ...
        if (!project.project_location?.toLowerCase().includes(location))
          return false;
      }

      const projectStartDate =
        project.date_start_the_project || project.date_start;
...
export const useProjectsWithFilters = (): UseProjectsWithFiltersReturn => {
  ...
  // Create the filter function once and memoize it
  const filterProjects = useMemo(() => createFilterProjectsFunction(), []);
