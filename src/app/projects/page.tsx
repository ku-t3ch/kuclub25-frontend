...
export default function ProjectsPage() {
  ...
  const themeValues = useMemo(() => ({
    ...
    backgroundGradient1: getValueForTheme('bg-blue-500 opacity-30', 'bg-primary opacity-10'),
    backgroundGradient2: getValueForTheme('bg-purple-500 opacity-20', 'bg-teal-500 opacity-10'),
      ...
      return projects
        .map((project) => {
          ...
          try {
            ...
            return {
              id: project.id,
              title: project.project_name_th || project.project_name_en || 'ไม่ระบุชื่อโครงการ',
              start,
              end,
              allDay: isMultiDay,
  ...
  const filteredProjects = useMemo(() => {
    ...
    return calendarProjects.filter(project => 
      project?.originalProject && projectMatchesFilters(project.originalProject, activeFilters, getActivityTypes)
    );
  ...
  const goToNextMonth = useCallback(() => {
    ...
    const nextYear = selectedMonth === 11
  ...
  const goToPreviousMonth = useCallback(() => {
    ...
    const prevYear = selectedMonth === 0
      ? currentDate.getFullYear() - 1
      : currentDate.getFullYear();
  ...
  return (
    <motion.div
      className={combine("min-h-screen relative overflow-hidden", themeValues.containerBg)}
      initial="initial"
      animate="animate"
      exit="exit"
